...
# zhuoyan: Configure logging to output to console at the INFO level
...
def load_pretrained(
    ...
    if load_from == 'state_dict':
        ...
        # print(f'zhuoyan: Loading pretrained weights from Hugging Face hub ({pretrained_loc})')
    ...
    # print(f"zhuoyan: len(state_dict) {len(state_dict)}")
    # print(f"zhuoyan list(state_dict)[:5]: {list(state_dict)[:5]}")
...
def resolve_pretrained_cfg(
    ...
    if not pretrained_cfg:
        # print(f"not pretrained_cfg: {pretrained_cfg}")
        if pretrained_tag:
            # print(f"if pretrained_tag: {pretrained_tag}")
            model_with_tag = '.'.join([variant, pretrained_tag])
        pretrained_cfg = get_pretrained_cfg(model_with_tag)
    # print(f"zhuoyan pretrained_cfg: {pretrained_cfg}")
    if not pretrained_cfg:
        _logger.warning(
