...
from ..model import LayerNorm, AffineDropPath


def make_optimizer(worker, module, cfg):
    decay, no_decay = set(), set()
    whitelist_modules = (nn.Embedding, nn.Linear, nn.Conv1d, nn.Conv2d)
    blacklist_modules = (LayerNorm, nn.BatchNorm2d, AffineDropPath)

    named_modules = worker.named_modules(module)
    ...
    param_dict = {pn: p for pn, p in worker.named_parameters(module)}
...
def make_scheduler(optimizer, cfg):
    ...
    if cfg['sched_type'] == 'cosine':
        max_itrs = cfg['epochs'] * cfg['itrs_per_epoch']
        scheduler = LinearWarmupCosineAnnealingLR(
            optimizer,
            warmup_epochs=cfg.get('warmup_itrs', 0),
            max_epochs=max_itrs,
        )
